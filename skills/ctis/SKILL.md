...
Comments and names are the parts that slip. Writing the explanation in Turkish pulls them along with it, and a Turkish token inside a submitted file is the defect this rule exists to prevent. `# aramaya sağ yarıda devam et` becomes `# keep searching in the right half`; `CREATE TABLE ogrenci` becomes `CREATE TABLE student`; `Olasılık:` in a document row becomes `Probability:`. A Turkish noun in the request is not a licence to name the artifact after it: asked in Turkish for a table of ogrenci and ders, you still write `student` and `course`. Everything inside the block is English even when every sentence around it is not.
