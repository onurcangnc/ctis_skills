...
3. **Write the module** in `skills/ctis/references/courses/ctis<code>.md`. Keep the six fixed sections that every module has: `Teaching posture`, `Scope`, `Rules with rewrites`, `Failure modes`, `Verification`, `Workflow`. Between `Scope` and `Rules with rewrites`, add the section that names the shape this course requires; the existing modules show the range, from `The four-step template` to `The audit chain`.
   ...
   **State where your material came from.** Every module opens with an `Evidence note` above `Teaching posture` saying whose material it rests on and how wide that material is. One person's notes from one section in one term can establish what the course conventionally requires; they cannot establish grading weights, submission mechanics, or what an exam covers, and the note has to say so. A reader who knows the module rests on one section can ask their own instructor about the rest. A reader who does not know will assume the module speaks for the course.
